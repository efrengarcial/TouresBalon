...
<!-- @generated mapFile="com/touresbalon/transforms/xslDataMappingCancReser.map" md5sum="e92c7468d96c648d54f75a96016d55bb" version="7.0.500" -->
<!--
*****************************************************************************
...
<xsl:stylesheet
  ...
  <xsl:template match="body"  mode="map:xslDataMappingCancReser">
    <body>
      ...
      <out:cancelRoom>
        <out:reservation>
          <!-- a simple data mapping: "in2:cancelReservation/request/hotelID"(int) to "out:HotelID"(int) -->
          <out:HotelID>
            <xsl:value-of select="in2:cancelReservation/request/hotelID"/>
          </out:HotelID>
          <!-- a simple data mapping: "in2:cancelReservation/request/roomNumber"(int) to "out:RoomNumber"(int) -->
          <out:RoomNumber>
            <xsl:value-of select="in2:cancelReservation/request/roomNumber"/>
          </out:RoomNumber>
          <!-- a simple data mapping: "in2:cancelReservation/request/checkInDate"(date) to "out:CheckInDate"(dateTime) -->
          <out:CheckInDate>
            <!-- variables for custom code -->
            <xsl:variable name="checkInDate" select="in2:cancelReservation/request/checkInDate"/>
            <xsl:value-of select="MapUtils:dateToDateTime($checkInDate, '1970-01-01T00:00:00')"/>
          </out:CheckInDate>
          <!-- a simple data mapping: "in2:cancelReservation/request/checkOutDate"(date) to "out:checkOutDate"(dateTime) -->
          <out:checkOutDate>
            <!-- variables for custom code -->
            <xsl:variable name="checkOutDate" select="in2:cancelReservation/request/checkOutDate"/>
  ...
  <xsl:template name="map:xslDataMappingCancReser2">
    ...
    <out:cancelRoom>
      <out:reservation>
        <!-- a simple data mapping: "$body/in2:cancelReservation/request/hotelID"(int) to "out:HotelID"(int) -->
        <out:HotelID>
          <xsl:value-of select="$body/in2:cancelReservation/request/hotelID"/>
        </out:HotelID>
        <!-- a simple data mapping: "$body/in2:cancelReservation/request/roomNumber"(int) to "out:RoomNumber"(int) -->
        <out:RoomNumber>
          <xsl:value-of select="$body/in2:cancelReservation/request/roomNumber"/>
        </out:RoomNumber>
        <!-- a simple data mapping: "$body/in2:cancelReservation/request/checkInDate"(date) to "out:CheckInDate"(dateTime) -->
        <out:CheckInDate>
          <!-- variables for custom code -->
          <xsl:variable name="checkInDate" select="$body/in2:cancelReservation/request/checkInDate"/>
          <xsl:value-of select="MapUtils:dateToDateTime($checkInDate, '1970-01-01T00:00:00')"/>
        </out:CheckInDate>
        <!-- a simple data mapping: "$body/in2:cancelReservation/request/checkOutDate"(date) to "out:checkOutDate"(dateTime) -->
        <out:checkOutDate>
          <!-- variables for custom code -->
          <xsl:variable name="checkOutDate" select="$body/in2:cancelReservation/request/checkOutDate"/>
          <xsl:value-of select="MapUtils:dateToDateTime($checkOutDate, '1970-01-01T00:00:00')"/>
        </out:checkOutDate>
