...
<xsl:stylesheet
    ...
    <body>
      ...
      <out2:checkAvailabilityResponse>
        <response>
          ...
          <xsl:apply-templates select="in:validateAvailabilityResponse/in:validateAvailabilityResult/in:Room" mode="localRoomToContent_1118693113"/>
        </response>
      </out2:checkAvailabilityResponse>
    </body>
  </xsl:template>
  ...
  <xsl:template name="map:xslDataMappingAvailabilityResp2">
    ...
    <out2:checkAvailabilityResponse>
      <response>
        <!-- a for-each transform: "in:Room"(Room) to "content"(RoomResponse) -->
        <xsl:apply-templates select="$body/in:validateAvailabilityResponse/in:validateAvailabilityResult/in:Room" mode="localRoomToContent_1118693113"/>
      </response>
    </out2:checkAvailabilityResponse>
  </xsl:template>

  <!-- This rule represents a for-each transform: "in:Room" to "content".  -->
  <xsl:template match="in:Room"  mode="localRoomToContent_1118693113">
    <content>
